...
import { getMatchPostFilter } from './match-filter-url'
...
const Page: FunctionComponent<PageProps> = async ({ searchParams }) => {
  ...
  const filter = getMatchPostFilter(searchParams?.filter ?? null)
  const initialData =
    filter === 'mine'
      ? undefined
      : await new MatchPostRepository().getMatchPosts(1, 10, {
          movieTitle,
          filter,
        })
  ...
  return (
    <main className="min-h-page bg-background pb-5 text-foreground">
      <MatchContainer movieTitle={movieTitle} initialData={initialData} />
    </main>
  )
}
